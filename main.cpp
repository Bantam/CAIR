...
enum Arg_Param { INPUT_FILENAME = 0, GOAL_X, GOAL_Y, ADD_WEIGHT, OUTPUT_FILENAME, RESULT_TYPE, CONVOLUTION, WEIGHT_FILENAME, WEIGHT_SCALE, ENERGY_TYPE, THREAD_COUNT };
...
void Print_Usage()
{
	...
	cout << "  -A <add_weight>" << endl;
	cout << "      Default: 400" << endl;
...
int main( int argc, char * argv[] )
{
	...
	//the -A param
	temp = getArgParameter( ADD_WEIGHT, argc, argv );
	if( temp != NULL )
	{
		add_weight = atoi( temp );
	}
	else
	{
		add_weight = 400;
	...
	if( output_filename == NULL )
	{
		...
		case 0 :
			...
			break;
		case 1 :
			output_filename = "out_greyscale.bmp";
			break;
		case 2 :
	...
	{
	case 0 :
		CAIR( &Source, &Weights, goal_x, goal_y, add_weight, convolution, ener, &D_Weights, &Dest, NULL ); //try cancel_callback
		break;
	case 1 :
	...
	case 4 :
		...
		break;
	case 5 :
		CAIR_Removal( &Source, &Weights, AUTO, 1, add_weight, convolution, ener, &D_Weights, &Dest, NULL );
		break;
	case 6 :
		CAIR_HD( &Source, &Weights, goal_x, goal_y, add_weight, convolution, ener, &D_Weights, &Dest, NULL );
		break;
	}
